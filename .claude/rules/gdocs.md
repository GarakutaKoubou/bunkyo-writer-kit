...
## ルール1：既存ドキュメントはピンポイント上書き（日付削除は禁止）
- `save_to_gdocs.py` 実行時、article.json に `gdocs_url` があれば**そのドキュメントだけを上書き更新**する（他記事には一切触れない）
- `gdocs_url` が無ければ新規作成する
- **「同日付の旧ファイルを一括削除」は廃止した。** 日付はかぶるため、日付削除は同日付の別記事のドキュメントを巻き込み削除する重大バグになる（実際に発生済み）
...
# ② article_index.json を自動更新 + HTMLを再生成（ファイル名はIDから自動解決）
